...
def _query_terms(query: str) -> tuple[str, ...]:
    """Tokenize queries independently of punctuation/case.

    Natural-language dashboard questions commonly contain punctuation such as
    ``oportunidade?`` or currency notation such as ``Rfrom __future__ import annotations
...
`. Splitting only on
    ...
    """
    return tuple(term for term in dict.fromkeys(re.findall(r"\\w+", query.casefold())) if len(term) > 1)
