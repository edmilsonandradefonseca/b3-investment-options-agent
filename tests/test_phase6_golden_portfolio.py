...
from b3_agent.portfolio.intelligence import PositionIntelligenceEngine as ExposureEngine
from b3_agent.portfolio.lifecycle import PositionLifecycleEngine
from b3_agent.portfolio.pnl import PnlEngine
from b3_agent.portfolio.position_intelligence import PositionIntelligenceEngine as AssessmentEngine
...
def test_golden_long_stock():
    ...
    assessment = AssessmentEngine().assess((p,), as_of=AS_OF)[0]
...
def test_golden_covered_call():
    ...
    exposure = ExposureEngine().build_exposures(PortfolioContext(AS_OF, (stock, call)))[0]
...
def test_golden_put_plus_stock():
    ...
    exposure = ExposureEngine().build_exposures(PortfolioContext(AS_OF, (stock, put), cash=10000))[0]
...
def test_golden_call_plus_stock():
    stock = Position("stock", "PETR4", "STOCK", 1000, market_value=30000)
    call = option("call", "PETRJ384", -10, 36, "CALL", "PETR4")
    assessments = AssessmentEngine().assess((stock, call), as_of=AS_OF)
...
def test_golden_put_and_call_same_underlying():
    ...
    exposure = ExposureEngine().build_exposures(PortfolioContext(AS_OF, (stock, put, call)))[0]
    assert exposure.assignment_capital == 80000
    assert exposure.covered_call_contracts == 10
