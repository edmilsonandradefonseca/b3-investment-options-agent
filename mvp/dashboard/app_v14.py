...
with tabs[0]:
    ...
    with top_right:
        ...
        # Use a horizontal bar instead of a donut. It is more readable for
        # portfolio allocation and never misrepresents signed short-option values.
        ...
        allocation = allocation.sort_values("Valor", ascending=True)
        ...
        fig.update_traces(
            ...
            cliponaxis=False,
            hovertemplate="<b>%{y}</b><br>R$ %{x:,.0f}<br>%{customdata[0]:.1f}%<extra></extra>",
        )
        fig.update_layout(
            height=330,
            margin=dict(l=10, r=70, t=5, b=25),
            xaxis_title="Exposição (R$)",
            yaxis_title="",
            showlegend=False,
        )
        fig.update_xaxes(tickformat="~s", separatethousands=True)
