...
with tabs[0]:
    ...
    with top_right:
        ...
        allocation = pd.DataFrame(
            {"Categoria": ["Ações", "Opções", "Cash"], "Valor": [stock_value, option_value, cash_value]}
        )
        fig = px.pie(allocation, names="Categoria", values="Valor", hole=0.62)
        fig.update_layout(height=360, margin=dict(l=10, r=10, t=10, b=10), legend_title="")
        fig.update_traces(textinfo="percent", hovertemplate="%{label}<br>R$ %{value:,.2f}<extra></extra>")
        st.plotly_chart(fig, use_container_width=True)
