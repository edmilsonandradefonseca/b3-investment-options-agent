...
function PageHeader({title,subtitle,status}:{title:string;subtitle:string;status?:ReactNode}){return <div className="page-header"><div><h1>{title}</h1><p>{subtitle}</p></div>{status&&<span className="page-status">{status}</span>}</div>}
function QualityBadge({value}:{value?:string}){const normalized=(value??"").toUpperCase(); const cls=normalized==="VALIDATED"?"validated":normalized==="WARNING"?"warning":normalized==="REJECTED"?"rejected":normalized==="STAGED"?"staged":""; return <span className={"quality-badge "+cls}>{value??"Não informado"}</span>}
function ProvenanceBlock({source,asOf,quality}:{source?:string;asOf?:string;quality?:string}){return <div className="provenance-block"><div><label>Fonte</label><span>{source??"Não informado"}</span></div><div><label>Data-base</label><span>{asOf??"Não informado"}</span></div><div><label>Qualidade</label><span><QualityBadge value={quality}/></span></div></div>}
